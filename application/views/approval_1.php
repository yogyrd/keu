<?php
$admin_page_title = 'Approval Keuangan || Kas Pengeluaran';
        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form action="<?= base_url('approval_1'); ?>" method="post">
                ...
                <form action="<?= base_url('approval_1/update_status'); ?>" method="post">
                <div class="panel-body">

                    <table id="tabel_trans_kas" class="table table-condensed table-bordered table-hover table-striped">
                        <thead>
                        <tr>
                            ...
                            <th width="15%">Nilai Uang Pengajuan</th>
                            <th width="15%">Budget</th>
                            <th width="15%">Realisasi</th>
                            <th width="15%">Keterangan realisasi</th>
                        ...
                        <?php foreach($list_trans as $info){ ?>
                            <tr>
                                ...
                                <td align="right">
                                    <?php $rp = number_format($info->nilaimax,0,'.',','); echo 'Rp '.$rp;?>
                                    <input type="hidden" id="nilaimax_<?= $info->transoutid; ?>" value="<?= $info->nilaimax; ?>">
                                </td>
                                <td>
                                    <div class="input-group">
                                        <input type="text" id="strrealisasi_<?= $info->transoutid; ?>" class="form-control input-sm" value="<?= $info->nilaiuang; ?>">
                                        <input type="hidden" id="realisasi_<?= $info->transoutid; ?>" name="realisasi_<?= $info->transoutid; ?>" class="form-control input-sm" value="<?= $info->nilaiuang; ?>">
                                    </div>
                                </td>
                                <td>
                                    <input type="text" name="realisasiket_<?= $info->transoutid; ?>" class="form-control input-sm" />
                                </td>
                                ...
                                <script>
                                    ...
                                    document.getElementById("strrealisasi_<?= $info->transoutid; ?>").oninput = function(e) {
                                        if (this.value != "") {
                                        ...
                                        document.getElementById("realisasi_<?= $info->transoutid; ?>").value = this.value.replace(/,/g, "");

                                    }

                                    document.getElementById("strrealisasi_<?= $info->transoutid; ?>").onblur = function(e) {
                                        document.getElementById("realisasi_<?= $info->transoutid; ?>").value = this.value.replace(/,/g, "");
                                        var realisasi = parseFloat(document.getElementById("realisasi_<?= $info->transoutid; ?>").value);
                                        var nilaimax = parseFloat(<?= $info->nilaimax; ?>);
                                        if (realisasi > nilaimax) {
                                            alert('Nilai Realisasi Melebihi Budget');
                                            $('#realisasi_<?= $info->transoutid; ?>').val(<?= $info->nilaiuang; ?>);
                                            $('#strrealisasi_<?= $info->transoutid; ?>').val(<?= $info->nilaiuang; ?>);
                                        }
                                    }

                                    $(document).ready(function () {
                                        var tbrealisasi = document.getElementById("strrealisasi_<?= $info->transoutid; ?>");
                ...
                <form action="<?= base_url('approval_1/reject_transaksi'); ?>" method="post">
    ...
    <script type="text/javascript">
        ...
        function view_approved() {
            ...
            if (tgl1 > tgl2) {
                ...
                window.location.href="<?= base_url('approval_1/view_approved'); ?>?locid=" + loc + "&tgl1=" + tgl1 + "&tgl2=" + tgl2;
            }
        }
    </script>
    <!-- /.row -->
